 ...
 */
function isVisible(vNode, screenReader, recursed) {

  // document object should always be visible but is never a
  // virtual node
  if (typeof vNode.nodeType !== 'undefined' && vNode.nodeType === 9) {
    return true;
  }
  ...
  const { nodeType, nodeName } = vNode.props;
  ...
  if (screenReader && vNode.attr('aria-hidden') === 'true') {
    return false;
  }

  if (vNode.actualNode) {
    let el = vNode.actualNode;
