...
/**
 * Check if an element is hidden outside of a parents overflow.
...
function isHiddenByOverflow(el) {
  ...
  while (parent) {
    ...
    if (style.getPropertyValue('overflow') === 'hidden') {
      ...
      // perform an AABB (axis-aligned bounding box) collision
      // check to see if the rect is inside the parent overflow
      if (
        !(
...
function isVisible(el, screenReader, recursed) {
  ...
  if (
    ...
    (style.getPropertyValue('visibility') === 'hidden' ||
      (!screenReader && (isOffscreen(el) || isHiddenByOverflow(el))))
  ) {
    return false;
  }
