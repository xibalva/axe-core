...
export function setAudit(value) {
	axeAudit = value;
}
...
class Audit {
	...
	_constructHelpUrls(previous = null) {
		var version = (getVersion().match(/^[1-9][0-9]*\.[0-9]+/) || ['x.y'])[0];
...
function getHelpUrl({ brand, application, lang }, ruleId, version) {
	return (
		...
		brand +
		'/' +
		(version || getVersion().substring(0, getVersion().lastIndexOf('.'))) +
		'/' +
		ruleId +
		'?application=' +
