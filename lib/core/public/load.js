...
import { getAudit, setAudit } from '../base/audit';
	...
	switch (data.command) {
		...
		default:
			...
			if (
				audit &&
				audit.commands &&
				audit.commands[data.command]
			) {
				return audit.commands[data.command](data, callback);
			}
	}
}
...
function load(audit) {
	...
	respondable.subscribe('axe.ping', function(
		data,
		keepalive,
		respond
	) {
