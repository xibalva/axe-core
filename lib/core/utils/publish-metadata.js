...
function publishMetaData(ruleResult) {
	...
	const checksData = audit.data.checks || {};
	const rulesData = audit.data.rules || {};
	const rule = findBy(audit.rules, 'id', ruleResult.id) || {};

	ruleResult.tags = clone(rule.tags || []);

	const shouldBeTrue = extender(checksData, true);
	const shouldBeFalse = extender(checksData, false);
